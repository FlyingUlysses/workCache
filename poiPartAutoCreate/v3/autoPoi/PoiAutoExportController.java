...
public class PoiAutoExportController extends Controller{
    ...
    public void getPages(){
        renderJson(PoiAutoExport.me.getPages(getParaToInt("page"),getParaToInt("limit")));
    }
    ...
    public void getJoinTables(){
        renderJson(PoiAutoExport.me.getJoinTables(getPara("table_name")));
    }
    ...
    public void getColumns(){
        renderJson(PoiAutoExport.me.getColumns(getPara("table_name")));
    }
    ...
    public void editPart(){
        ExcelPart part = ExcelPart.me.findById(getParaToInt("id"));
        setAttr("part", part);
        renderJsp("edit_part.jsp");
    }
    
    /**
     * 编辑excel模板页，获取模板cell信息
     * @Description:
     */
    public void getCellArray(){
        renderJson(Db.find("select id,cellname,width,height,startRow,startColumn,isMerge,endRow,endColumn,property from excel_cells where template=? ",getParaToInt("template_id")));
    }
    ...
    public void savePart(){
        ...
        JSONArray cellArray = JSON.parseArray(getPara("cell_array"));
        if (excelId!=null) {
            try {
                ...
                .set("isFixed", 0)
                .set("sort", partSort);
                if (StringUtils.isNotEmpty(sheetCat) && sheetCat.equals("all")) {
                    part.set("isFixed", 1)
                    .set("sheet", partName);
                }
                if (partId!=null && templateId!=null){ 
                ...
                for (int i = 0; i < cellArray.size(); i++) {
                    ...
                    try {width = cell.getInteger("width");} catch (Exception e) {}
                    ...
                    try {isMerge = cell.getString("isMerge");} catch (Exception e) {}
